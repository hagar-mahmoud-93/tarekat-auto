...
import { TarikaFundsStatusCashInvestmentAccountsClient } from '../../api/clients/cashDivisionAPIs/tarika-funds-status-cashInvestmentAccounts.client';
...
test.describe('Inheritance seeder', () => {
  ...
  test('Cash division - Investment Accounts @smoke', async ({ seederPage, request, page: adminPage }) => {
    ...
    await test.step('Simulate Tarika funds status and complete heir inquiries and account selection', async () => {
      await new TarikaFundsStatusCashInvestmentAccountsClient(request).simulate(result, divisionId);
